...
public class PlayerMainFrm extends javax.swing.JFrame {
    ...
    public PlayerMainFrm(PlayerCtr ro, Player pl) {
        ...
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_ORDER_RANK, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_PLAYERS_ONLINE, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_LIST_FRIEND, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_GET_REQUEST, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_LIST_GROUP, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_CHECK_FRIEND, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_REQUEST_FRIEND, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_CHECK_REQUEST_FRIEND, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_DELETE_REQUEST, this));
        ...
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_CHECK_GROUP, this));
        mySocket.getActiveFunction().add(new ObjectWrapper(ObjectWrapper.REPLY_CREATE_GROUP, this));

        this.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                mySocket.sendData(new ObjectWrapper(ObjectWrapper.UPDATE_PLAYER_STATUS, player));
        ...
        if (data.getData() instanceof ArrayList<?>) {
            ...
            DefaultTableModel tableModelWait = new DefaultTableModel(value1, columnNames1) {
                ...
                public Class getColumnClass(int column1) {
                    tbl_wait.getColumnModel().getColumn(0).setPreferredWidth(10);
                    tbl_wait.getColumnModel().getColumn(1).setPreferredWidth(520);
    ...
    public void tbl_rankFriend(ObjectWrapper data) {
        ...
        if (data.getData() instanceof ArrayList<?>) {
            ...
            String[] columnNames2 = {"", "Name", "Win Count", "Core", "Rank"};
            ...
            for (int i = 0; i < list2.size(); i++) {
                if (list2.get(i).getStatus().equals("1")) {
                    ...
                    value2[i][2] = list2.get(i).getWin_count();
                    value2[i][3] = list2.get(i).getCore();
                    value2[i][4] = list2.get(i).getRank();
                } else {
                    value2[i][0] = offline;
                    value2[i][1] = list2.get(i).getFullname();
                    value2[i][2] = list2.get(i).getWin_count();
            ...
            DefaultTableModel tableModelRank = new DefaultTableModel(value2, columnNames2) {
                @Override
                public Class getColumnClass(int column1) {
                    tbl_rank.getColumnModel().getColumn(0).setPreferredWidth(10);
                    tbl_rank.getColumnModel().getColumn(1).setPreferredWidth(330);
                    tbl_rank.getColumnModel().getColumn(2).setPreferredWidth(70);
                    tbl_rank.getColumnModel().getColumn(3).setPreferredWidth(50);
                    tbl_rank.getColumnModel().getColumn(4).setPreferredWidth(50);
    ...
    public void tbl_listFirend(ObjectWrapper data) {
        ...
        if (data.getData() instanceof ArrayList<?>) {
            ...
            for (int i = 0; i < list4.size(); i++) {
                if (list4.get(i).getIdPlayer1() == player.getId_player() || list4.get(i).getIdPlayer2() == player.getId_player()) {
            ...
            for (int i = 0; i < listF.size(); i++) {
                if (listF.get(i).getIdPlayer1() == player.getId_player()) {
            ...
            DefaultTableModel tableModelFriend = new DefaultTableModel(value3, columnNames3) {
                @Override
                public Class getColumnClass(int column1) {
                    tbl_friends.getColumnModel().getColumn(0).setPreferredWidth(10);
                    tbl_friends.getColumnModel().getColumn(1).setPreferredWidth(520);
                    return (column1 == 0) ? Icon.class : Object.class;
                }
        ...
        if (data.getData() instanceof ArrayList<?>) {
            ...
            DefaultTableModel tableModelOnline = new DefaultTableModel(value, columnNames) {
                ...
                public Class getColumnClass(int column) {
                    tbl_players_online.getColumnModel().getColumn(0).setPreferredWidth(10);
                    tbl_players_online.getColumnModel().getColumn(1).setPreferredWidth(520);
    ...
    private void initComponents() {
        ...
        jTabbedPane1.addTab("Danh sách người chơi online", jPanel1);
    ...
    private void btnExitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExitActionPerformed
        // mySocket.updatePlayerstatus("0", player.getId_player());
        this.dispose();
    ...
    private void btnAcceptActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAcceptActionPerformed
        ...
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.ADD_FRIEND, friend));
    ...
    private void btnAddFriendActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddFriendActionPerformed
        ...
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.REQUEST_FRIEND, request));
    ...
    private void tbl_rankMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_rankMouseClicked
        ...
        if (row < tbl_rank.getRowCount() && row >= 0 && column < tbl_rank.getColumnCount() && column >= 0) {
            temp1 = list2.get(row);
            if (player.getId_player() == temp1.getIdplayer()) {
                btnAddFriend.setEnabled(false);
            } else {
                ...
                mySocket.sendData(new ObjectWrapper(ObjectWrapper.CHECK_FRIEND, r));
                mySocket.sendData(new ObjectWrapper(ObjectWrapper.CHECK_REQUEST_FRIEND, r));
            }
        }
    ...
    public void deleteRequest(ObjectWrapper data) {
        if (data.getData().equals("ok")) {
    ...
    private void btnDenyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDenyActionPerformed
        // TODO add your handling code here:
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.DELETE_REQUEST, requestModel));
    ...
    public void deleFriend(ObjectWrapper data) {
        if (data.getData().equals("ok")) {
            if (player.getId_player() == delFriend.getIdPlayer1()) {
                JOptionPane.showMessageDialog(this, "Da xoa " + delFriend.getNamePlayer2() + " khoi danh sach ban be");
            } else if (player.getId_player() == delFriend.getIdPlayer2()) {
    ...
    private void btnDeleteFriendActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteFriendActionPerformed
        // TODO add your handling code here:
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.DELETE_FRIEND, delFriend));
    ...
    private void tbl_GroupMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_GroupMouseClicked
        ...
        if (row < tbl_Group.getRowCount() && row >= 0 && column < tbl_Group.getColumnCount() && column >= 0) {
            ...
            new GroupDetailFrm(mySocket, player, listGroup.get(row)).setVisible(true);
            mySocket.sendData(new ObjectWrapper(ObjectWrapper.LIST_MEMBER, group));
            mySocket.sendData(new ObjectWrapper(ObjectWrapper.LIST_APPROVAL, group));
            group.setIdplayer(player.getId_player());
            group.setNameGroup(group.getNameGroup());
            mySocket.sendData(new ObjectWrapper(ObjectWrapper.CHECK_LEAVE, group));
            mySocket.sendData(new ObjectWrapper(ObjectWrapper.CHECK_JOIN, player));
            mySocket.sendData(new ObjectWrapper(ObjectWrapper.CHECK_JOIN_APPROVAL, group));
        }
    ...
    private void btnCreateGroupActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCreateGroupActionPerformed
        ...
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.CREATE_GROUP, group));
